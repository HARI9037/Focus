            ...
            try{
                when(call.method){
                    "status"->{val store=NightStore(this);result.success(mapOf(
                        ...
                        "nights" to NightAudit(this).rows().let{a->(0 until a.length()).map{i->val o=a.getJSONObject(i);o.keys().asSequence().associateWith{k->o.get(k)}}},
                    ...
                    "discipline"->{DisciplineStore(this).configure(call.argument<String>("rules")?:"[]");FocusAccessibilityService.instance?.configurationChanged();result.success(null)}
                    "focus"->{DisciplineStore(this).setFocus(call.argument<String>("id"),call.argument<Number>("end")?.toLong()?:0);FocusAccessibilityService.instance?.configurationChanged();result.success(null)}
                    "events"->{FocusEventScheduler.update(this,call.argument<String>("events")?:"[]");result.success(null)}
                    "permission"->{
