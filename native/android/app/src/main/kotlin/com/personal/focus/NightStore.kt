...
class NightStore(val context: Context) {
    ...
    fun overrideExpiry(): Long {
        ...
        if (policy().intervalContaining(Instant.now(),ZoneId.systemDefault())==null) {
    ...
    fun beginOverride(reason: String, emergency: Boolean = false) {
        ...
        check(prefs.edit().putLong("overrideExpiry",elapsed+duration).putLong("overrideStartedElapsed",elapsed)
