...
void validateEntry(Entry e) {
  const states = {
    ...
    'milestone': {'inbox', 'planned', 'in_progress', 'completed', 'cancelled'},
    ...
    'habit': {'active'},
    'session': {'active', 'completed', 'interrupted'},
    'override': {'active', 'completed'},
  };
  if (e.id.isEmpty ||
      e.id.length > 100 ||
      e.title.trim().isEmpty ||
      e.title.length > 240 ||
      e.body.length > 1000000 ||
      e.priority < 0 ||
      e.priority > 3 ||
  ...
  if (tags != null &&
      (tags is! List || tags.length > 200 || tags.any((t) => t is! String))) {
    throw const FormatException('Invalid tags');
  }
  if (e.data['pinned'] != null && e.data['pinned'] is! bool) {
  ...
  if (e.kind == 'task') {
    if (![
      ...
      'weekly',
      'monthly',
    ].contains(e.data['recurrence'] ?? 'none')) {
  ...
  if (e.kind == 'goal') {
    ...
    if (target is! num ||
        ...
        !current.isFinite ||
        current < 0 ||
        e.data['unit'] is! String) {
      throw const FormatException('Invalid goal');
    }
  }
  if (e.kind == 'habit' && e.data['days'] != null) {
    final days = e.data['days'];
    if (days is! List || days.length > 50000) {
      throw const FormatException('Invalid habit days');
    }
    for (final day in days) {
      if (day is! String ||
          DateTime.tryParse(day) == null ||
          dayKey(DateTime.parse(day)) != day) {
  ...
  if (e.kind == 'session') {
    ...
    if (started is! String ||
        DateTime.tryParse(started) == null ||
        (e.state != 'active' &&
            (ended is! String || DateTime.tryParse(ended) == null))) {
      throw const FormatException('Invalid session interval');
    }
    final targetEnd = e.data['targetEnd'];
    if (targetEnd != null &&
        (targetEnd is! String || DateTime.tryParse(targetEnd) == null)) {
