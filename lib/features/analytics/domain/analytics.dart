...
class ExplainedScore {
  ...
  double? get value {
    ...
    return available.fold<double>(0, (s,p) => s + p.value!.clamp(0,1) * p.weight) / total * 100;
  }
  double get coverage {
    final total = parts.fold<double>(0, (s,p) => s+p.weight);
    if (total == 0) return 0;
    return parts.where((p) => p.value != null).fold<double>(0,(s,p) => s+p.weight)/total;
  }
}
class AnalyticsEngine {
  static int focusSeconds(Iterable<Entry> entries, DateTime start, DateTime end) {
    // Session seconds are sliced at local period boundaries, never assigned wholly to start day.
    var result = 0;
    for (final e in entries.where((e) => e.kind == 'session' && !e.deleted && e.state != 'active')) {
  ...
  static ExplainedScore productivity({required int completed, required int planned,
    required int focusSeconds, required int targetSeconds}) => ExplainedScore([
    ScorePart('Planned tasks completed', planned > 0 ? completed/planned : null, 0.5),
    ScorePart('Focus target reached', targetSeconds > 0 ? focusSeconds/targetSeconds : null, 0.5),
  ]);
  static ExplainedScore discipline({int? usageSeconds, required int budgetSeconds,
    double? nightCompliance}) => ExplainedScore([
      ScorePart('Phone budget', usageSeconds == null || budgetSeconds <= 0 ? null :
        (usageSeconds <= budgetSeconds ? 1 : budgetSeconds/usageSeconds), 0.6),
  ...
  static String? usageChange(double current, double previous) {
    ...
    if (percent == 0) return 'Phone usage was unchanged across these observed periods.';
  ...
  static int streak(Set<String> completedDays, DateTime now) {
    ...
    if (!completedDays.contains(dayKey(cursor))) cursor = DateTime(cursor.year,cursor.month,cursor.day-1);
    var count = 0;
